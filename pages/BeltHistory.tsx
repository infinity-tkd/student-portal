...
const BeltHistory: React.FC = () => {
 ...
 theme: "white",
 text: "text-slate-800 dark:text-white", subtext: "text-slate-600 dark:text-slate-400",
 accent: "bg-slate-200",
 gradient: "from-slate-100 to-slate-200", dot: "bg-slate-300",
 pill: "bg-slate-50 dark:bg-brand-black/50 text-slate-700 dark:text-slate-400 border-slate-100 dark:border-white/5"
 };
 ...
 pill: "bg-white dark:bg-slate-900/60 text-amber-700 border-yellow-200"
 };
 ...
 pill: "bg-white dark:bg-slate-900/60 text-emerald-700 border-emerald-200"
 };
 ...
 text: "text-red-900", subtext: "text-red-600/70",
 accent: "bg-red-600",
 gradient: "from-red-500 to-indigo-600", dot: "bg-red-300",
 pill: "bg-white dark:bg-slate-900/60 text-red-700 border-red-200"
 };
 ...
 pill: "bg-white dark:bg-slate-900/60 text-rose-700 border-rose-200"
 };
 ...
 pill: "bg-white dark:bg-slate-900/60 text-orange-800 border-orange-200"
 };
 ...
 text: "text-white", subtext: "text-slate-600 dark:text-slate-400",
 accent: "bg-slate-800",
 gradient: "from-slate-700 to-black", dot: "bg-slate-600",
 pill: "bg-slate-800 text-slate-500 dark:text-slate-300 border-slate-700"
 };
 ...
 <h1 className="text-2xl font-black text-brand-black dark:text-white tracking-tight">Belt Journey <span className="text-amber-500">.</span></h1>
 <p className="text-[10px] font-bold text-slate-600 dark:text-slate-400 tracking-wide uppercase mt-1">Path to Black Belt Mastery</p>
 </div>

 {/* EMPTY STATE ERROR HANDLING */}
 {history.length === 0 ? (
 <div className="px-4 md:px-0">
 <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl border-2 border-dashed border-slate-200 dark:border-white/10 p-12 text-center">
 <div className="w-16 h-16 rounded-full bg-slate-50 dark:bg-brand-black/50 flex items-center justify-center mx-auto mb-4">
 <Award className="text-slate-500 dark:text-slate-300" size={32} />
 </div>
 <h3 className="text-sm font-black uppercase tracking-widest text-slate-600 dark:text-slate-400">No History Found</h3>
 <p className="text-slate-600 dark:text-slate-400 text-xs mt-2">Your promotion history will appear here once you begin your testing journey.</p>
 </div>
 </div>
 ) : (
 <div className="relative mx-4 md:mx-0 md:px-4 max-w-3xl mx-auto">
 {/* Connection Line */}
 <div className="absolute left-[23px] top-6 bottom-6 w-0.5 bg-gradient-to-b from-slate-300 via-slate-300 dark:from-white/10 dark:via-white/10 to-transparent"></div>
 ...
 <div className={`w-12 h-12 rounded-full bg-white dark:bg-[#1a1a1a] border-[4px] border-white dark:border-[#111111] shadow-sm flex items-center justify-center transition-transform duration-500 group-hover:scale-110 ring-1 ring-slate-50 dark:ring-transparent`}>
 ...
 {/* Watermark */}
 <Award className={`absolute -right-4 -bottom-4 w-24 h-24 ${s.theme === 'black' ? 'text-white/5' : 'text-slate-900 dark:text-white/5'} -rotate-12 pointer-events-none`} />
 ...
 {hasCert ? (
 <div className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest ${s.theme === 'black' ? 'text-red-300' : 'text-red-600'} group-hover:underline decoration-2 underline-offset-4`}>
 <ExternalLink size={12} />
 View Certificate
 </div>
 ) : (
 <div className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest ${s.subtext}`}>
 ...
 <div className="bg-white dark:bg-[#1a1a1a] w-full max-w-[95vw] md:max-w-3xl lg:max-w-5xl xl:max-w-6xl rounded-2xl overflow-hidden shadow-2xl relative z-10 animate-slide-up flex flex-col h-[85vh] md:h-[90vh] border border-slate-200 dark:border-white/10">

 {/* Modal Header */}
 <div className="p-4 border-b border-slate-200 dark:border-white/10 flex justify-between items-center bg-white dark:bg-[#1a1a1a] sticky top-0 z-20">
 <div className="flex items-center gap-3">
 <div className="w-10 h-10 rounded-xl bg-slate-50 dark:bg-brand-black/50 flex items-center justify-center text-slate-600 dark:text-slate-400">
 <Award size={20} />
 </div>
 <div>
 <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-wide">{selectedCert.rank} Certificate</h3>
 <p className="text-[10px] text-slate-600 dark:text-slate-400 font-bold uppercase tracking-widest">ID: {selectedCert.certificateId || '---'}</p>
 </div>
 </div>
 <button
 onClick={() => setSelectedCert(null)}
 className="w-9 h-9 rounded-full bg-slate-50 dark:bg-brand-black/50 text-slate-600 dark:text-slate-400 flex items-center justify-center hover:bg-slate-100 dark:hover:bg-white/10 transition-colors"
 >
 <X size={18} />
 </button>
 </div>
 ...
 <div className="bg-slate-100 dark:bg-white/10 p-4 md:p-8 flex-1 flex items-center justify-center overflow-hidden relative">
 {!imgError ? (
 <img
 ...
 <AlertCircle size={40} className="mx-auto text-slate-500 dark:text-slate-300 mb-2" />
 <p className="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase">Image Unavailable</p>
 <p className="text-[10px] text-slate-600 dark:text-slate-400 mt-1">We couldn't load the preview. Try downloading it.</p>
 </div>
 )}
 </div>

 {/* Actions Footer */}
 <div className="p-4 grid grid-cols-2 gap-3 bg-white dark:bg-[#1a1a1a] border-t border-slate-200 dark:border-white/10">
 <button
 onClick={() => selectedCert.certificateFileId && handleDownload(selectedCert.certificateFileId)}
 className="flex items-center justify-center gap-2 py-3.5 rounded-xl bg-slate-100 dark:bg-white/10 text-slate-600 text-xs font-black uppercase tracking-wide hover:bg-slate-200 transition-colors active:scale-95"
 >
 <Download size={16} />
 Download
 </button>
 ...
 className="flex items-center justify-center gap-2 py-3.5 rounded-xl bg-brand-black text-white text-xs font-black uppercase tracking-wide hover:bg-red-900 transition-colors active:scale-95 relative overflow-hidden"
 >
 <Share2 size={16} />
 Share
 </button>
...
const ShareModal: React.FC<{
 ...
 <div className="bg-white dark:bg-[#1a1a1a] w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl p-6 relative z-10 animate-slide-up shadow-2xl border border-slate-200 dark:border-white/10">

 <div className="flex justify-between items-center mb-6">
 <h3 className="text-lg font-black text-slate-800 dark:text-white tracking-tight">Share Certificate</h3>
 <button onClick={onClose} className="p-2 rounded-full hover:bg-slate-100 dark:hover:bg-white/10 text-slate-600 dark:text-slate-400 hover:text-slate-600 transition-colors">
 <X size={20} />
 </button>
 </div>

 {/* Link Copy Section */}
 <div className="bg-slate-50 dark:bg-brand-black/50 p-3 rounded-xl flex items-center gap-3 mb-6 border border-slate-100 dark:border-white/5">
 <div className="flex-1 overflow-hidden">
 <p className="text-[10px] text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider mb-1">Certificate Link</p>
 <p className="text-xs text-slate-600 truncate font-mono select-all">{url}</p>
 </div>
 <button
 onClick={handleCopy}
 className={`p-2.5 rounded-lg transition-all active:scale-95 shrink-0 ${copied ? 'bg-emerald-500 text-white shadow-lg shadow-emerald-200' : 'bg-white dark:bg-slate-900 text-slate-600 border border-slate-200 dark:border-white/10 hover:border-slate-300'}`}
 >
 ...
 <span className="text-[10px] font-bold text-slate-700 dark:text-slate-400">Facebook</span>
 </button>
 ...
 <span className="text-[10px] font-bold text-slate-700 dark:text-slate-400">Messenger</span>
 </button>
 ...
 <span className="text-[10px] font-bold text-slate-700 dark:text-slate-400">WhatsApp</span>
 </button>
 ...
 <span className="text-[10px] font-bold text-slate-700 dark:text-slate-400">Telegram</span>
 </button>
 ...
 <span className="text-[10px] font-bold text-slate-700 dark:text-slate-400">Instagram</span>
 </button>
 ...
 <span className="text-[10px] font-bold text-slate-700 dark:text-slate-400">TikTok</span>
 </button>

 <button onClick={handleNativeShare} className="flex flex-col items-center gap-2 group">
 <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-white/10 flex items-center justify-center text-slate-700 dark:text-slate-400 group-hover:bg-slate-200 transition-all shadow-sm border border-slate-200 dark:border-white/10">
 <Share2 size={24} />
 </div>
 <span className="text-[10px] font-bold text-slate-700 dark:text-slate-400">More</span>
 </button>
 </div>
 ...
 <button
 onClick={onClose}
 className="w-full py-4 text-center text-slate-700 dark:text-slate-400 font-bold border-t border-slate-200 dark:border-white/10 uppercase text-xs hover:text-slate-800 dark:hover:text-white transition-colors"
 >
 Cancel
 </button>
 </div>
